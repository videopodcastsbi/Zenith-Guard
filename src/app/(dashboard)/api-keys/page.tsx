          ...
          <DialogContent className="bg-[#111118] border-gray-800 text-white">
            ...
            <form action={handleAddKey} ref={formRef} className="space-y-4 py-4">
              <div className="space-y-2">
                ...
                <Input name="name" required placeholder="e.g. Production Server 1" className="bg-[#1a1a24] border-gray-800" />
              </div>
              <div className="space-y-2">
                <label className="text-sm font-medium">Select Game</label>
                <select name="game_id" required className="flex h-10 w-full rounded-md border border-gray-800 bg-[#1a1a24] px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2">
                  <option value="">-- Choose a Game --</option>
                  {games.map(g => (
                    <option key={g.id} value={g.id}>{g.name}</option>
                  ))}
                </select>
                {games.length === 0 && <p className="text-xs text-red-400 mt-1">Please create a game first before generating an API key.</p>}
              </div>
              <Button type="submit" disabled={isPending || games.length === 0} className="w-full bg-cyan-600 hover:bg-cyan-700">
              ...
              {loading ? (
                ...
                keys.map((item) => (
                  <TableRow key={item.id} className="border-gray-800 hover:bg-gray-800/30">
                    ...
                    <TableCell className="font-mono text-gray-400 flex items-center">
                      {item.key_hash.substring(0, 8)}...{item.key_hash.substring(item.key_hash.length - 4)}
                      <Button onClick={() => copyToClipboard(item.key_hash)} variant="ghost" size="icon" className="ml-2 h-6 w-6 text-gray-500 hover:text-white">
                        {copiedKey === item.key_hash ? <Check className="h-3 w-3 text-green-400" /> : <Copy className="h-3 w-3" />}
                      </Button>
                    </TableCell>
                    <TableCell className="text-gray-400">{item.games?.name || 'Unknown'}</TableCell>
                    <TableCell className="text-gray-400">{new Date(item.created_at).toLocaleDateString()}</TableCell>
                    <TableCell className="text-gray-400">{item.last_used_at ? new Date(item.last_used_at).toLocaleDateString() : 'Never'}</TableCell>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-8">
        <Card className="bg-[#111118]/50 backdrop-blur border-gray-800">
          <CardHeader>
            ...
            <CardDescription>Add this script to ServerScriptService</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="bg-[#0a0a0f] p-4 rounded-md border border-gray-800 overflow-x-auto">
    ...
    HttpService:PostAsync(
        ...
        <Card className="bg-[#111118]/50 backdrop-blur border-gray-800">
          <CardHeader>
            ...
            <CardDescription>Direct API access documentation</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="flex items-center space-x-4 p-3 bg-gray-900/50 rounded-md border border-gray-800">
              <Badge className="bg-blue-600">GET</Badge>
              <span className="font-mono text-sm text-gray-300">/v1/games/status</span>
            </div>
            <div className="flex items-center space-x-4 p-3 bg-gray-900/50 rounded-md border border-gray-800">
              <Badge className="bg-green-600">POST</Badge>
              <span className="font-mono text-sm text-gray-300">/v1/events</span>
            </div>
            <div className="flex items-center space-x-4 p-3 bg-gray-900/50 rounded-md border border-gray-800">
              <Badge className="bg-yellow-600 text-black hover:bg-yellow-500">PUT</Badge>
              <span className="font-mono text-sm text-gray-300">/v1/players/:id/status</span>
            </div>
          </CardContent>
        </Card>
      </div>
